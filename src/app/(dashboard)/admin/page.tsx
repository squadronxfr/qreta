...
import {AdminStoreProvider, useAdminStore} from "@/providers/admin-store-provider";
...
import {Search, Shield, Store as StoreIcon, ArrowRight, AlertTriangle} from "lucide-react";
                                            ...
                                            <TableRow key={user.uid} className="hover:bg-slate-50">
                                                ...
                                                <TableCell>
                                                    <div className="flex items-center gap-2">
                                                        {user.firstname && user.lastname ? (
                                                            <span
                                                                className="text-slate-700">{user.firstname} {user.lastname}</span>
                                                        ) : (
                                                ...
                                                <TableCell>
                                                    {user.subscription?.cancelAtPeriodEnd ? (
                                                        ...
                                                        <Badge
                                                            className={STATUS_STYLES[user.subscription?.status] || "bg-slate-100 text-slate-600"}>
                                                            {STATUS_LABELS[user.subscription?.status] || user.subscription?.status || "N/A"}
                                                        </Badge>
                                                    )}
                                                </TableCell>
                                                <TableCell>
                                                    {user.stores.length > 0 ? (
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="font-medium">{user.stores.length}</span>
                                                            ...
                                                            {user.stores.length > 2 && (
                                                                <span
                                                                    className="text-xs text-slate-400">+{user.stores.length - 2}</span>
                                                            )}
                                                        </div>
                                                    ) : (
